...
class Assistant(Agent):
    ...
    async def stt_node(self, audio: AsyncIterable[rtc.AudioFrame], model_settings) -> Optional[AsyncIterable]:
        async def passthrough_audio():
            async for frame in audio:
                # Custom audio preprocessing can be done here
                yield frame

        events = await super().stt_node(passthrough_audio(), model_settings)
        return events
